...
| 插件版本 | DSH 版本 | 状态 |
| --- | --- | --- |
| `1.9.16`（当前发布版本） | `0.1.5-rc.3`（最新稳定版）**与**预览线（`0.1.6-alpha.2`、`0.1.7-alpha.*`、`0.1.7-rc.2`） | **支持** —— 声明范围是 `>=0.1.5-rc.2 <0.2.0-0 \|\| >=0.1.6-alpha.1 <0.2.0-0 \|\| >=0.1.7-alpha.1 <0.2.0-0`，CI 对稳定线与两条预览线各安装一次并各跑一遍完整测试。官方桌面端（bundled `0.1.7-rc.2`，win32）也在覆盖范围内 |
| `1.9.15` | 同上 | 支持（已被取代） |
| `1.9.14` | 同上 | 支持（已被取代） |
| `1.9.13` | 同上 | 支持（已被取代） |
| `1.9.12` | 同上 | 支持（已被取代） |
