...
#### 更正（发布后浏览器复核，2026-09-21）

上面第 4、5 条（`#6` / `#8`）**未通过发布后的真机复核**，如实记账：

- **`#6` 忙碌期的编辑被静默丢弃 —— 未修好。** 状态层的守护（`update()` 在 busy 时拒绝改动）已生效，
  但**控件没有全部转只读**：编辑器加了 `readOnly`，描述框与行开关仍可输入。
  实测（`lab/probe-ui-issue6.mjs`，网络延迟拉 4000ms）：忙碌窗口内 39 个控件仍有 0 个被禁用、
  往返期间敲入的文字依旧消失。**下一步**：让面板内所有会改草稿的控件（描述框、名称、行开关）
  一起随 `busy` 转只读/禁用，并把探针的选择器改成语言无关（目前按英文界面写的）。
- **`#8` 「按本线修复」被下一次保存撤销 —— 未修好。** 已经把 `repairedIds` 交给页面并折进草稿，
  但实测仍然是"修复后磁盘正确、保存一次又被打开"。**根因未定**，下一步需要在页面里追踪
  `overrides` 在 `reload()` 前后的实际取值。

其余三条（`#2` / `#3` / `#7`）**已通过**：`#2` `#3` 由 `test/composition-edge.test.mjs` 的
逐行 id 扫描守住；`#7` 由真机复核确认（英文界面删除确认弹窗不再出现全角括号）。
