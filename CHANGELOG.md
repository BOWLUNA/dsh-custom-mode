...
- **尺寸统一为 800x800**：四张 README 图都裁到设置弹窗本身的大小（1440x900 视口下弹窗正好
  800x800），不再是"这张 1600x1600、那张 3360x2100"的参差排版。
- **体积降到三分之一**：拍摄从 deviceScaleFactor 2 改成 **1**。GitHub 会把正文里的图缩到栏宽，
  2 倍图只是让体积翻两番。实测同样四张图 **~750 KB → ~223 KB**（44–76 KB 一张），
  而 800 宽的图按原尺寸显示，字反而更清楚。
- **只保留 README 真正要用的四张**：深色与英文界面仍然**真的切换并断言**（观察值写进
  `observed.json`），只是不再各存一张重复画面。真机会话的那张实证图移到
  `docs/实测记录.md` §0，不再占 README 的位置。
