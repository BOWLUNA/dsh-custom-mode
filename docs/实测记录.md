...
```
# 修复前：每次启动 tui 都打印这一行（与 P0 变砖时的报错一模一样，纯误导）
dsh: warning: 1 entry did not activate
custom-prompt-editor (dsh-custom-prompt-editor): pending (waiting for services: webServer, agentPresets)

# 修复后：不再 pending（插件改成作用域内等待服务）；install.sh 也会当场说明
    注意: profile "tui" 的组合里没有 web 服务器，因此没有设置页可用。
          仍然可用的是：模式本身（新建会话时可选）、以及 custom_prompt 工具
          想要图形化设置页，请装进 web profile：./install.sh --profile web
```

> 后半句"模式本身仍然可用"在 tui 上其实**不成立**（没有 `agent-presets`，模式选不到）。
> 这段文案按 profile 分别判断会更准确，已记在待办里。
...
# 修复前：dsh-custom-prompt-editor      ← 仍在（指向仓库的软链）
# 修复后：（无输出）
```
