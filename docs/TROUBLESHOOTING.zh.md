...
## 18. 页面报「宿主内部错误：无法定位出厂基础模式」，或基础模式与插件开关变灰

### 症状

设置页能打开、助手列表也在，但是：

- **1.9.13 之前** —— 本该显示模式名称、基础模式药丸、插件开关与提示词编辑器的位置只有一行红字
  `宿主内部错误：无法定位出厂基础模式…`；`GET /api/custom-mode/state?id=…` 返回 **500**（`internalError`），
  `POST /api/custom-mode/state` 返回 **400**（`renderFailed`）；
- **1.9.13 起** —— 同一页出现一行告警，基础模式与插件开关显示为不可用，而系统提示词照常读取、**照常保存**。

两种情况里模式本身都是好的：它在新会话选择器里，用它起的会话也正常。

### 原因

插件是靠在基础模式的**出厂组成**上做手术来合成模式的，而它找不到那份文本。dsh 有两条线发布这份声明：
基于文件的 `@deepseek-ai/dsh-agent-presets` 包（0.1.2 … 0.1.6），以及宿主自己交出的
`agentPresets.readDocument(<mode>).content`（0.1.7+）。1.9.12 只认第一条，于是在原生 `0.1.7-rc.2`
（官方桌面端内置的那条线）上每次读取都抛错。详见 `docs/ARCHITECTURE.zh.md` §18。

1.9.13 起失败变成类型化的 `baseCompositionUnavailable` 而不是 500，并且页面降级：提示词可单独保存，
带着行开关的保存会被明确拒绝并给出提示，而不是悄悄丢掉。

### 怎么办

1. **升级到 1.9.13 或更新** —— 这就是全部修法：
   `dsh plugin --profile web add dsh-custom-mode@1.9.13`，然后重启 dsh。
2. **去宿主日志里看它试过哪些路。** 插件会把每一次尝试都点名：
   `custom-mode: 取不到这些基础模式的出厂组成 —— standard: …`。如果出现
   `agentPresets.readDocument() 不可用`，说明宿主早于声明式注册表，你所在的线不在本插件声明支持的范围内。
3. **只有在必须留在 1.9.12 时**：把 `DSH_SHIPPED_PRESETS_DIR` 指向任意一个含
   `<mode>/agent.cordis.yml` 的旧线 presets 目录（老包本体或其副本）。它优先级最高，且一旦设置就**跳过**
   发现逻辑 —— 所以写错的路径不会被静默忽略，会在日志里明确失败。
4. **如果你是维护者**：这就是测试套件三个版本都没看见的东西。夹具由 `test/run.mjs` 注入，0.1.7 那条路
   从未被执行；而 CI 在 0.1.7 上连夹具都装不上（`ERESOLVE`），那几条腿在任何测试跑起来之前就红了。
   两处在 1.9.13 都修了 —— 但教训仍然成立：**夹具被注入的绿色套件，不是真实安装的证据。**
