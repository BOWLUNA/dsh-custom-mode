...
### 18.2 未结论（如实记录，含原因与复现路径）

**"另一个进程改写 `prompt.md` 之后，同一条会话的下一步是否立刻生效"** —— 这一条没拿到结论：

- 两次尝试事后确认**被发送的会话不是我们的模式**（一次是遗留的旧会话，一次新会话要先在模式选择器里
  选模式）。此后驱动加了"发送前校验模式，不是就拒发"的守卫，于是这些回合被拒绝发送，**不构成证据**；
- 决定性的一次（用设置页接口写入带 `[[MARKER-9C2E]]` 的格式规则后提问）在开始时 headless Chrome 卡死，
  消息没有发出。

复现路径是明确的（§0 那次成功实测用的就是同一手法：**植入格式规则、看模型行为**，而不是问模型
"你的系统提示词是什么" —— 后者会被拒绝或只给出概括）：**先确认会话模式是「自定义模式」**，然后两轮 ——
写入规则后问一次（应出现标记），撤掉规则再问一次（不应出现）。本机 headless Chrome 反复卡死，
这条留给下一次或人工复测。
